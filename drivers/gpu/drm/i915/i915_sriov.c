...
static void vf_tweak_device_info(struct drm_i915_private *i915)
{
	...
	struct intel_display_runtime_info *drinfo = DISPLAY_RUNTIME_INFO(display);
	 ...
	 * info for VF.
	 */
	memcpy(drinfo, &display->info.__device_info->__runtime_defaults, sizeof(*drinfo));
}
...
static void pf_restore_vfs_irqs(struct drm_i915_private *i915, unsigned int num_vfs)
{
	...
	for_each_gt(gt, i915, gt_id) {
		...
		for (vfid = 1; vfid <= num_vfs; vfid++)
			raw_reg_write(gt->uncore->regs, vf_master_irq(i915, vfid), GEN11_MASTER_IRQ);
	}
}
...
static void intel_runtime_pm_enable_interrupts(struct drm_i915_private *i915)
{
	struct intel_gt *gt;
	unsigned int id;

	for_each_gt(gt, i915, id)
		intel_guc_enable_interrupts(&gt->uc.guc);
}
